...
if ($submit) {
  ...
  {
	  // by billing date
	  $numtaxes = add_taxdetails($DB, $billingdate, NULL, 'invoice', $batchid, $organization_id);
	  $numservices = add_servicedetails($DB, $billingdate, NULL,'invoice', $batchid, $organization_id);
  }
  else
  {
      // by billing id
	  $numtaxes = add_taxdetails($DB, NULL, $bybillingid,'invoice', $batchid, NULL);
	  $numservices = add_servicedetails($DB, NULL, $bybillingid,'invoice', $batchid, NULL);
  }
