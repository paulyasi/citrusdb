...
// Copyright (C) 2003-2010  Paul Yasi (paul at citrusdb.org)
...
if (!isset($base->input['org_id'])) { $base->input['org_id'] = 1; }

$day1 = $base->input['day1'];
$day2 = $base->input['day2'];
$org_id = $base->input['org_id'];
...
Organization: <select name=org_id>";
...
while ($myresult = $result->FetchRow()) {
  // print a menu with all organizations
  $myid = $myresult['id'];
...
if ($day1) {
  ...
  $query = "SELECT ROUND(SUM(bd.paid_amount),2) AS CategoryTotal, 
		...
		AND bd.taxed_services_id IS NULL AND g.id = '$org_id' 
  ...
  $query = "SELECT ph.billing_amount, ph.invoice_number, ".
    ...
    "AND ph.payment_type = 'discount' AND bi.organization_id = '$org_id'";
