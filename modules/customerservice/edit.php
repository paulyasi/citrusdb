...
if ($base->input['save']) {
	...
	if ( ($street != $old_street) OR ($city != $old_city) OR ($state != $old_state) OR ($zip != $old_zip) 
		OR ($country != $old_country) OR ($phone != $old_phone) OR ($fax != $old_fax) 
		OR ($contact_email != $old_contact_email) )
	{
